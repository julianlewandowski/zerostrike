...
export default function TacticalMap() {
  ...
  useEffect(() => {
    ...
    const onUp = () => {
      ...
      if (box && (box.ne[0] - box.sw[0] > 0.001 || box.ne[1] - box.sw[1] > 0.001)) {
        ...
        // Use cameraForBounds so we can update controlled viewState (fitBounds gets overwritten by old state)
  ...
  // When switching to 3D with pending bounds (from draw), fit bounds after style loads
  useEffect(() => {
    const map = mapRef.current;
  ...
  // Sync viewState from map move (fitBounds updates programmatically)
  useEffect(() => {
    const map = mapRef.current;
    ...
    const onMoveEnd = () => {
      ...
      setViewState((prev) => ({ ...prev, longitude: c.lng, latitude: c.lat, zoom: z, pitch: p, bearing: b }));
    };
